...
//Alias Type
//We use Alias Type to create a new name for an existing type.
//This is useful for simplifying complex types or creating more readable code.
type Combinable = number | string;
type conversion = "as-number" | "as-text"; // This creates an alias 'Combinable
...
function combine(
  ...
  if (
    ...
    throw new Error(
      "Invalid input types. Both inputs must be either numbers or strings."
    );
  }
}

const combinedResult = combine("5", "6", "as-number"); // Here, we can pass a number and a string, and specify the resultConversion as 'as-number'
// This will add the number and the string (after converting the string to a number)
console.log(combinedResult);

const combinedResultString = combine("3", "10", "as-text"); // Here, we can pass two strings and specify the resultConversion as 'as-text'
